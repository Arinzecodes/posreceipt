document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!deviceSerial || !virtualTerminalId || !transactionRRN) {
        console.error('Missing required parameters');
        return;
      }

  const apiUrl =
    `https://performance-report.staging.redpay.africa/api/v1/Reports/transaction/terminal/receipt?deviceSerial=${deviceSerial}&virtualTerminalId=${virtualTerminalId}&transactionRRN=${transactionRRN}`;
  ...
  try {
    ...
    if (data.status && data.data) {
      ...
      document.getElementById('merchant-id').textContent =
      ...
      document.getElementById('rrn').textContent =
        transaction.retrievalReferenceNumber || 'N/A';
